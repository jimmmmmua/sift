 ...
 * 2003/09/27 | PICTURE0.BIN(DLL)pÉü¢ ( by ­[Ýñ )
 * 2003/09/28 | eíoOtBNXœ­ÌÅK» ( by H.Kawai )
 *
 */
...
int decode0_JPEG(struct DLL_STRPICENV *env, int size, UCHAR *fp0, int b_type, UCHAR *buf, int skip)
{
    ...
    //	if (jpeg->width == 0)
    //		return 8;
    /* decode0ÅÍinfoµÄ©çÄÎêéÌÅA±êÍÈ¢ */
...
unsigned short get_bits(JPEG *jpeg, int bit)
{
    ...
    while (remain <= 16)
    {
        ...
        c = *jpeg->fp++;
        if (c == 0xff)
        { // }[JG[ðh®œßAFF -> FF 00 ÉGXP[v³êÄé
            if (jpeg->fp >= jpeg->fp1)
            {
                ret = 0;
                goto fin;
...
int jpeg_dqt(JPEG *jpeg)
{
    ...
    while (size > 0)
    {
        ...
        if (c & 0xf8)
        {
            ...
            for (i = 0; i < 64; i++)
            {
                jpeg->dqt[j][i] = jpeg->fp[0]; /* ºÊÍÇÝÌÄÄ¢é */
                jpeg->fp += 2;
            }
            size += -64 * 2;
        }
...
int jpeg_dht(JPEG *jpeg)
{
    ...
    while (len > 0)
    {
        ...
        tc = (val >> 4) & 0x0f; // e[uNX(DC/AC¬ªZN^)
        th = val & 0x0f;        // e[uwb_(œÚÌv[©)
...
int jpeg_decode_mcu(JPEG *jpeg, UCHAR *zigzag_table)
{
    ...
    // mcu_width x mcu_heightTCYÌubNðWJ
...
void jpeg_decode(JPEG *jpeg, UCHAR *rgb, int b_type)
{
    ...
    for (v = 0; v < v_unit; v++)
    {
        for (h = 0; h < h_unit; h++)
        {
            ...
            {
                // RSTm}[Jð·ÁòÎ·(FF hoge)
                // hogeÍÇÝòÎµÄéÌÅAFFàòÎ·
